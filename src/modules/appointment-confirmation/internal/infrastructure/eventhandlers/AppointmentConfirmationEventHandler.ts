...
export class AppointmentConfirmationEventHandler implements OnModuleInit {
    constructor(@Inject(IEventBus) private readonly eventBus: IEventBus, private readonly appointmentConfirmationHandler: AppointmentConfirmationHandler) { }
    ...
    public handle(payload: AppointmentBookedDto): void {
        console.log('AppointmentConfirmationEventHandler: AppointmentBooked event received');
        this.appointmentConfirmationHandler.handle(AppointmentConfirmationMapper.toAppointmentConfirmationDetails(payload));
    }
}